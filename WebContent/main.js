...
function handleSelectSuggestion(suggestion) {
	...
	var url = "singleMovie.jsp" + "?Title=" + suggestion["value"]
	var curURL = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split('/')[1] + "/" + encodeURI(url);
	console.log(curURL);
	loadPage(curURL);
...
$('#autocomplete').autocomplete({
    ...
    lookup: function (query, doneCallback) {
    		if(query.length > 2)
    		{
    			...
    			{
    				var data = searches[query]
    				console.log("look up " + query  + " on cashed browser")
    				handleLookupAjaxSuccess(data, query, doneCallback) 
    			}
    			else
    			{
...
function handleNormalSearch(query) {
	...
	var url = "movieListSearch.jsp" + "?title=" + encodeURI(query) + "&year=&director=&starName=";
	var curURL = window.location.protocol + "//" + window.location.host + "/" + window.location.pathname.split('/')[1] + "/" + encodeURI(url);
	loadPage(curURL);
}
